...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--only_return_keys",
        action='store_true',
        help="if to only return the keys (which is a list of list of files' directory).",
    )
    parser.add_argument(
        "--num_splits",
        type=int,
        default=1,
        help="the number of splits into which the (flat) list of file directories will be split. This has effect only if `only_return_keys` is `True`.",
    )
